...
import { select, selectAll, attr, data } from 'd3-selection'
import { scaleLinear, domain, range, nice } from 'd3-scale'
import { axisBottom, scale, tickValues, tickFormat } from 'd3-axis'
...
export default class SummaryChart extends Component {
  ...
  render() {
    ...
    let bar = chart.selectAll('g.bar').data(data.summary)

    let xAxis = axisBottom().scale(xScale)

    if (data.sample_size < 8) {
      xAxis
        .tickValues(ticks(0, data.sample_size, data.sample_size))
        .tickFormat(format('d'))
    }
    ...
    let g = bar
      ...
      .append('g')
      .attr('transform', (d, i) => {
        return 'translate(0,' + (i * barHeight + 26) + ')'
      })
