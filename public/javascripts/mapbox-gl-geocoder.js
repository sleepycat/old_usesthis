...
Geocoder.prototype = mapboxgl.util.inherit(mapboxgl.Control, {
  ...
  _geocode: function(q, callback) {
    ...
    if (this.options.proximity){
      url + '&proximity=' + this.options.proximity.join();
    }
    if (this.options.country){
      url + '&country=' + this.options.country;
    }
    if (this.options.types){
      url + '&types=' + this.options.types;
    }

    //TODO: this used to cancel existing requests.
    //It's not clear how to do that yet with isomorphic-fetch.
